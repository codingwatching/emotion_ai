...
function simulation(band = 'Alpha', dominant = 'serotonin_like') {
  return {
    ...
    display: { basis: 'published_affect_state', brainwave: band, activation: 0.3, dominant_channel: dominant },
  };
}
function ui(api = {}) {
  ...
  for (const name of ['emotionStatusElement', 'emotionDetailsElement', 'emotionIntensityElement',
    ...
    'simRevisionBadge', 'simPolicyDesc', 'simValence', 'simCuriosity', 'simAffiliation', 'simLoad', 'simCauses']) {
...
test('invalid tone classification does not clear valid controller indicators', () => {
  ...
  assert.equal(manager.emotionStatusElement.textContent, 'Unknown');
  assert.equal(manager.brainwaveValueElement.textContent, 'Alpha');
...
test('changes in the controller update both indicators and clear missing data', () => {
  ...
  assert.equal(manager.brainwaveValueElement.textContent, 'Gamma');
